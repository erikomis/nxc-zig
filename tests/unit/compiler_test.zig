...
const freeDiagnostics = util.freeDiagnostics;
const deinitCompileResult = util.deinitCompileResult;
...
const expectContains = util.expectContains;
const expectNotContains = util.expectNotContains;
...
test "ts: assertions fixture strips legacy and modern syntax" {
    const result = try compiler.compileFile(
        ...
        .{ .parser = .{ .syntax = .typescript }, .jsx = false, .target = .es2024 },
        std.testing.io,
        std.testing.allocator,
    );
    defer deinitCompileResult(std.testing.allocator, result);
...
test "tsconfig paths: parsed and applied" {
    ...
    const ts = (try config_mod.readTsConfig(tmp_path, std.testing.io, alloc)) orelse return error.NoConfig;
    defer {
...
fn compileFixture(path: []const u8) !compiler.CompileResult {
    return compiler.compileFile(
        path,
        .{ .parser = .{ .syntax = .typescript }, .jsx = false, .target = .es2024, .transform = .{ .decorator_metadata = false } },
        std.testing.io,
        std.testing.allocator,
    );
}
...
test "ts-support: const enum emits valid runtime enum" {
    ...
    try expectContainsText(out, "export const x = 1 /* Flags.Read */;");
    try std.testing.expect(std.mem.indexOf(u8, out, "var Flags =") == null);
...
test "tsc compat: erases broad type-only syntax from runtime emit" {
    const out = try compileTs(
        \\interface Box<T extends string = string> { readonly value?: T }
        \\type Mapper<T> = { [K in keyof T]?: T[K] };
        \\type Pair = readonly [name: string, count?: number, ...rest: boolean[]];
        \\declare global { interface Window { appVersion: string } }
        \\function identity<const T>(value: T): T { return value }
        \\const value = identity<string>("ok") satisfies string;
        \\const length = (value as string)!.length;
        \\export { value, length };
    );
    defer std.testing.allocator.free(out);

    try expectContains(out, "function identity(value)");
    try expectContains(out, "const value = identity(\"ok\");");
    try expectContains(out, "const length = value.length;");
    try expectContains(out, "export { value, length };");
    try expectNotContains(out, "interface Box");
    try expectNotContains(out, "type Mapper");
    try expectNotContains(out, "satisfies");
    try expectNotContains(out, " as string");
}

test "tsc compat: strips type-only imports and exports" {
    const out = try compileTs(
        \\import type { User, Role as UserRole } from "./types";
        \\import { type Config, runtime } from "./runtime";
    ...
    try expectContains(out, "import { runtime } from \"./runtime\";");
    try expectContains(out, "export { runtime };");
    try expectContains(out, "runtime();");
    try expectNotContains(out, "./types");
    try expectNotContains(out, "type Config");
}

test "tsc compat: emits runtime enums and inlines const enums" {
    const out = try compileTs(
        \\enum Status { Ready, Busy = 3, Done }
        \\const enum Flag { Off, On = 7, Label = "label" }
        \\export const ready = Status.Ready;
        \\export const on = Flag.On;
        \\export const label = Flag.Label;
    );
    defer std.testing.allocator.free(out);

    try expectContains(out, "var Status = (function(_E) {");
    try expectContains(out, "_E[_E[\"Ready\"] = 0] = \"Ready\";");
    try expectContains(out, "_E[_E[\"Busy\"] = 3] = \"Busy\";");
    try expectContains(out, "export const ready = Status.Ready;");
    try expectContains(out, "export const on = 7 /* Flag.On */;");
    try expectContains(out, "export const label = \"label\" /* Flag.Label */;");
    try expectNotContains(out, "var Flag");
}

test "tsc compat: lowers namespace runtime exports" {
    const out = try compileTs(
        \\namespace Api {
        \\  export const version = "1";
        \\  export function name(): string { return "api" }
        \\  const internal = true;
        \\}
    ...
    try expectContains(out, "const Api = (function(_N) {");
    try expectContains(out, "const version = \"1\";");
    try expectContains(out, "_N.version = version;");
    try expectContains(out, "function name()");
    try expectContains(out, "_N.name = name;");
    try expectContains(out, "export const version = Api.version;");
}

test "tsc compat: constructor parameter properties become assignments" {
    const out = try compileTs(
        \\class User {
        \\  constructor(public id: string, private readonly token: string, name: string) {
        \\    this.name = name;
        \\  }
        \\}
    );
    ...
    try expectContains(out, "constructor(id, token, name) {");
    try expectContains(out, "this.id = id;");
    try expectContains(out, "this.token = token;");
    try expectContains(out, "this.name = name;");
    try expectNotContains(out, "public id");
    try expectNotContains(out, "readonly token");
}

test "tsc compat: overload signatures and abstract members are erased" {
    const out = try compileTs(
    ...
    try expectContains(out, "class Reader {");
    try expectContains(out, "read(value) {");
    try expectContains(out, "return value;");
    try expectNotContains(out, "abstract source");
    try expectNotContains(out, "read(value: string)");
}

test "tsc compat: predicates and assertion signatures are runtime-neutral" {
    const out = try compileTs(
        \\function isString(value: unknown): value is string { return typeof value === "string" }
        \\function assertString(value: unknown): asserts value is string { if (!isString(value)) throw new Error("bad") }
        \\export const ok = isString("x");
    );
    defer std.testing.allocator.free(out);

    try expectContains(out, "function isString(value)");
    try expectContains(out, "function assertString(value)");
    try expectContains(out, "export const ok = isString(\"x\");");
    try expectNotContains(out, "value is string");
    try expectNotContains(out, "asserts value");
}
...
test "ts: declare support matrix fixture compiles" {
    const result = try compiler.compileFile(
        ...
        .{ .parser = .{ .syntax = .typescript, .decorators = true }, .jsx = false, .target = .es2020 },
        std.testing.io,
        std.testing.allocator,
    );
