...
pub fn run(
    ...
    // Only legacy decorators are lowered today. Stage-3 decorators must either
    // be implemented or rejected earlier by a diagnostic before enabling this path.
    if (cfg.decorators and cfg.legacy_decorator) try decorators.transformDecorators(arena, alloc, .{
    ...
    try paths.resolveFullPaths(arena, alloc, io, cfg.paths_cfg);
    if (cfg.esmodule_interop) {
        try modules.transformEsmInterop(arena, alloc, io, cfg.src_file, cfg.import_interop, program_id);
    }
