...
fn buildProps(
    ...
    for (attrs) |attr| {
        switch (attr) {
            ...
            .named => |n| {
                const raw_val = n.value orelse try arena.push(.{ .bool_lit = .{ .value = true, .span = n.span } });
                const val = blk: {
                    if (arena.get(raw_val).* == .jsx_expr_container) {
                        const expr = arena.get(raw_val).jsx_expr_container.expr;
                        break :blk expr orelse try arena.push(.{ .bool_lit = .{ .value = true, .span = n.span } });
                    }
                    break :blk raw_val;
                };
