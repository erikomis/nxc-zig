...
pub const Codegen = struct {
    ...
    fn emitCommentsBefore(self: *Codegen, pos: u32) !void {
        ...
        while (self.comment_index < self.opts.comments.len and self.opts.comments[self.comment_index].span.start <= pos) {
            ...
            // Fix 5: Record source map mapping for comments
            if (self.source_map != null) {
                const comment_node = ast.Node{ .empty_stmt = .{ .span = self.opts.comments[self.comment_index].span } };
                try self.recordNodeMapping(&comment_node);
            }
    ...
    fn emitRemainingComments(self: *Codegen) !void {
        ...
        while (self.comment_index < self.opts.comments.len) {
            ...
            // Fix 5: Record source map mapping for remaining comments
            if (self.source_map != null) {
                const comment_node = ast.Node{ .empty_stmt = .{ .span = self.opts.comments[self.comment_index].span } };
                try self.recordNodeMapping(&comment_node);
            }
    ...
    fn genDispatch(self: *Codegen, node: *const ast.Node) anyerror!void {
        switch (node.*) {
            .program => try self.genProgram(node.program),
            .block => try self.genBlock(node.block),
            .expr_stmt => {
                try self.gen(node.expr_stmt.expr);
                try self.w(";");
            },
            .empty_stmt => {},
            .raw_js => {
                if (!isCommentRaw(node.raw_js.code)) try self.w(node.raw_js.code);
            },
            .var_decl => try self.genVarDecl(node.var_decl),
            .fn_decl => try self.genFnDecl(node.fn_decl),
            .class_decl => try self.genClassDecl(node.class_decl),
            .return_stmt => try self.genReturn(node.return_stmt),
            .if_stmt => try self.genIf(node.if_stmt),
            .for_stmt => try self.genFor(node.for_stmt),
            .while_stmt => try self.genWhile(node.while_stmt),
            .switch_stmt => try self.genSwitch(node.switch_stmt),
            .try_stmt => try self.genTry(node.try_stmt),
            .throw_stmt => {
                try self.w("throw ");
                try self.gen(node.throw_stmt.argument);
                try self.w(";");
            },
            .break_continue => try self.genBreakContinue(node.break_continue),
            .debugger_stmt => try self.w("debugger;"),
            .labeled_stmt => {
                try self.gen(node.labeled_stmt.label);
                try self.w(": ");
                try self.gen(node.labeled_stmt.body);
            },
            .import_decl => try self.genImport(node.import_decl),
            .export_decl => try self.genExport(node.export_decl),
            .ident => try self.w(node.ident.name),
            .num_lit => try self.w(node.num_lit.raw),
            .str_lit => try self.w(node.str_lit.raw),
            .bool_lit => try self.w(if (node.bool_lit.value) "true" else "false"),
            .null_lit => try self.w("null"),
            .undefined_ref => try self.w("undefined"),
            .regex_lit => try self.w(node.regex_lit.raw),
            .binary_expr => try self.genBinary(node.binary_expr),
            .unary_expr => try self.genUnary(node.unary_expr),
            .update_expr => try self.genUpdate(node.update_expr),
            .assign_expr => try self.genAssign(node.assign_expr),
            .call_expr => try self.genCall(node.call_expr),
            .member_expr => try self.genMember(node.member_expr),
            .new_expr => try self.genNew(node.new_expr),
            .new_target => try self.w("new.target"),
            .seq_expr => try self.genSeq(node.seq_expr),
            .cond_expr => try self.genCond(node.cond_expr),
            .arrow_fn => try self.genArrow(node.arrow_fn),
            .fn_expr => try self.genFnExpr(node.fn_expr),
            .class_expr => try self.genClassExpr(node.class_expr),
            .object_expr => try self.genObject(node.object_expr),
            .array_expr => try self.genArray(node.array_expr),
            .template_lit => try self.genTemplate(node.template_lit),
            .tagged_template => {
                try self.gen(node.tagged_template.tag);
                try self.genTemplate(self.arena.get(node.tagged_template.quasi).template_lit);
            },
            .spread_elem => {
                try self.w("...");
                if (self.arena.get(node.spread_elem.argument).* == .cond_expr) {
                    try self.w("(");
                    try self.gen(node.spread_elem.argument);
                    try self.w(")");
                } else try self.gen(node.spread_elem.argument);
            },
            .yield_expr => try self.genYield(node.yield_expr),
            .await_expr => {
                try self.w("await ");
                try self.gen(node.await_expr.argument);
            },
            .rest_elem => {
                try self.w("...");
                try self.gen(node.rest_elem.argument);
            },
            .assign_pat => {
                try self.gen(node.assign_pat.left);
                try self.w(" = ");
                try self.gen(node.assign_pat.right);
            },
            .object_pat => try self.genObjectPat(node.object_pat),
            .array_pat => try self.genArrayPat(node.array_pat),
            .ts_enum => try self.genEnum(node.ts_enum),
            ...
            .ts_non_null => try self.gen(node.ts_non_null.expr),
            .ts_as_expr => try self.gen(node.ts_as_expr.expr),
            .ts_satisfies => try self.gen(node.ts_satisfies.expr),
            .ts_instantiation => try self.gen(node.ts_instantiation.expr),
            .ts_type_assert => try self.gen(node.ts_type_assert.expr),
            .ts_namespace => try self.genNamespaceDecl(node.ts_namespace),
            .jsx_element, .jsx_fragment, .jsx_text, .jsx_expr_container, .jsx_name, .jsx_member_expr => {},
            .import_meta => try self.w("import.meta"),
            .import_call => {
                try self.w("import(");
                try self.gen(node.import_call.source);
                if (self.opts.keep_import_attributes and node.import_call.attributes.len > 0) {
                    try self.w(", { with: { ");
                    for (node.import_call.attributes, 0..) |a, i| {
                        if (i > 0) try self.w(", ");
    ...
    fn genScopedStatements(self: *Codegen, body: []const NodeId) !void {
        ...
        for (body) |stmt| {
            if (self.isOmittedStatement(stmt)) {
                self.consumeMergeDeclForStmt(stmt);
                continue;
            }

            if (emitted_any) try self.nl();
            try self.gen(stmt);
    ...
    fn isOmittedStatement(self: *const Codegen, stmt: NodeId) bool {
        return switch (self.arena.get(stmt).*) {
            .empty_stmt => true,
            .raw_js => |raw| isCommentRaw(raw.code),
            else => false,
        };
    }
    ...
    fn genBlock(self: *Codegen, b: ast.Block) !void {
        ...
        var emitted_any = false;
        for (b.body) |stmt| {
            if (self.isOmittedStatement(stmt)) continue;
            try self.nl();
            try self.gen(stmt);
            emitted_any = true;
        }
    ...
    fn genVarDeclNoSemi(self: *Codegen, v: ast.VarDecl) !void {
        // Fix 3: await using handled - is_await flag outputs "await " before "using "
    ...
    fn classMemberKeyIsPrivate(self: *Codegen, key_id: NodeId) bool {
        return switch (self.arena.get(key_id).*) {
            .ident => |id| id.name.len > 0 and id.name[0] == '#',
            else => false,
        };
    }

    fn genClassMemberKey(self: *Codegen, m: ast.ClassMember) !void {
        // Fix 1: Preserve # prefix for private names
        if (self.classMemberKeyIsPrivate(m.key)) {
            // The ident already includes # in its name (parser stores `#x` as ident name `#x`)
            try self.gen(m.key);
        } else if (m.is_computed) {
            try self.w("[");
            try self.gen(m.key);
            try self.w("]");
        } else {
        ...
        switch (m.kind) {
            ...
            .method, .getter, .setter => {
                ...
                if (m.value) |v| {
                    ...
                    try self.w(") ");
                    // Fix 4: Emit return type annotation for accessor methods
                    if (m.kind == .getter) {
    ...
    fn genEnum(self: *Codegen, e: ast.TsEnumDecl) !void {
        ...
        for (e.members) |m| {
            ...
            if (m.init) |init_id| {
                switch (self.arena.get(init_id).*) {
                    .str_lit => |s| {
                        try self.wf("_E[\"{s}\"] = ", .{key});
                        try self.w(s.raw);
                        try self.w(";");
                        next_auto_value = null;
                    },
                    .num_lit => |n| {
                        try self.wf("_E[_E[\"{s}\"] = {s}] = \"{s}\";", .{ key, n.raw, key });
                        next_auto_value = (std.fmt.parseInt(i64, n.raw, 10) catch 0) + 1;
                    },
                    else => {
                ...
                const value = next_auto_value orelse 0;
                try self.wf("_E[_E[\"{s}\"] = {d}] = \"{s}\";", .{ key, value, key });
                next_auto_value = value + 1;
            }
        }
        try self.nl();
