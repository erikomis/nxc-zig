...
fn parseSource(env: napi_env, info: napi_callback_info) callconv(.c) napi_value {
    ...
    var parse_result = compiler.parse(source, "input.ts", cfg, std.heap.page_allocator) catch return null;
    defer parse_result.deinit(std.heap.page_allocator);

    const has_program = if (parse_result.program_id != null) "true" else "false";
    const msg = std.fmt.allocPrint(std.heap.page_allocator, "parsed (program={s}, diagnostics={d})", .{ has_program, parse_result.diagnostics.len }) catch return null;
