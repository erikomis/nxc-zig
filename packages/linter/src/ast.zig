...
/// Whether the source file is a Script (no ESM) or Module (ESM).
...
pub const Program = struct {
    ...
    body: []NodeId,
    /// Whether the source is a script or module.
...
pub const NodePool = struct {
    ...
    pub fn deinit(self: *NodePool) void {
        for (self.slabs.items) |slab| {
            self.alloc.free(slab);
        }
        self.slabs.deinit(self.alloc);
    }
...
pub const Arena = struct {
    ...
    pub fn init(alloc: std.mem.Allocator) Arena {
        return .{ .nodes = .empty, .alloc = alloc, .pool = NodePool.init(alloc) };
    }
    ...
    pub fn pushWithPool(self: *Arena, node: Node) !NodeId {
        const id: NodeId = @intCast(self.nodes.items.len);
        const ptr = try self.pool.allocNode();
        ptr.* = node;
        try self.nodes.append(self.alloc, node);
        return id;
    }
